...
public class FFT {
    ...
    public void addData(double d) {
        if (dSize >= data.length) {
            for (int i = 1; i < data.length; i++) {
                data[i - 1] = data[i];
            }
            data[data.length - 1] = new Complex(d, 0);
        } else {
            data[dSize] = new Complex(d, 0);
            dSize++;
        }
    }
    ...
    public double getFFTMag() {
        ...
        Complex[] tempFFt = fft(data);
        for (Complex du : tempFFt) {
            Log.e("FFT Data ","FFT: "+du.abs());
            if (du.abs() > max) {
                max = du.abs();
            }
        }
